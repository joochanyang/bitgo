...
- ✅ **회고 자동화 완료**(`cmd/agent/retrospect.go`, 커밋 `93996b7`): 페이퍼는 실 포지션이 없어 저장된 episode의 `EntryPrice`·방향·`StopLossPct`/`TakeProfitPct`로 **시뮬 청산** 판정. 순수헬퍼 `evalClose(ep, price)`(롱: SL=entry×(1−SL%)·TP=entry×(1+TP%), 숏 대칭, pnl 방향반영) + `runCycle`이 **council 호출 전에** open episode를 `memory.Close`로 닫음 → 그 다음 `Recall`이 최신 결과를 council에 학습 반영. 단위테스트 6개(롱/숏 SL·TP·밴드내유지·비진입) + 통합 close 경로 실측(TP@0.53→pnl+6%·멱등). ⚠️**틱단위 근사**(캔들 고저 미반영 — 실 executor 단계서 정밀화).
- 🔜 **다음 = 장시간 관찰/프롬프트 튜닝 또는 실 executor**: 4h봉이라 자연 사이클 4시간 주기. 진입신호 보려면 ①변동성 큰 심볼 추가(config `symbols`) 또는 ②`cmd/agent`를 백그라운드 상주(홈서버/run 스크립트)로 며칠 돌려 진입의도→회고청산까지 episode 누적. 그 후 실 executor(PlaceOrder·**MinOrderQty 배선 필수**=ponytail 마커).
